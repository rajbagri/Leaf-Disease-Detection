...
def validate_image_quality(img_array: np.ndarray) -> dict:
    ...
    if brightness > 230:
        return {"valid": False, "reason": "Image is too bright / overexposed. Please upload a better-lit photo."}

    # Contrast check (std deviation of pixel values)
    contrast = gray.std()
    if contrast < 15:
        return {"valid": False, "reason": "Image has very low contrast. Please upload a clearer photo."}

    return {"valid": True, "blur_score": round(blur_score, 2), "brightness": round(float(brightness), 2)}


# ── Step 2: Leaf Validation (reject non-leaf images) ─────────────────────────
def validate_is_leaf(img_array: np.ndarray) -> dict:
    """
    Uses green channel dominance + HSV green mask coverage
    to determine if the image likely contains a leaf.
    """
    # Check green channel dominance
    r, g, b = img_array[:,:,0], img_array[:,:,1], img_array[:,:,2]
    green_dominant_pixels = np.sum((g > r) & (g > b))
    total_pixels = img_array.shape[0] * img_array.shape[1]
    green_ratio = green_dominant_pixels / total_pixels

    # HSV green + yellow-green range mask (covers healthy and diseased leaves)
    ...
    leaf_coverage = np.sum(leaf_mask > 0) / total_pixels
...
# ── Step 3: Leaf Segmentation (isolate leaf, remove background) ──────────────
def segment_leaf(img_array: np.ndarray) -> np.ndarray:
    """
    Uses GrabCut to isolate the leaf from background.
    ...
    try:
        ...
        # GrabCut with center rectangle (assumes leaf is roughly centered)
        ...
        # If mask is too empty, skip segmentation
        if final_mask.sum() < (h * w * 0.05):
            return img_array

        # Apply mask — background becomes white (neutral for model)
        ...
        # Never crash on segmentation — just return original
        return img_array


# ── Step 4: Auto Image Enhancement ───────────────────────────────────────────
def enhance_image(img: Image.Image) -> Image.Image:
    """
    Applies CLAHE on luminance channel, denoising, sharpening,
    and saturation boost to make disease spots more visible.
    """
    ...
    # Slight color/saturation boost — makes disease spots more distinguishable
    img = ImageEnhance.Color(img).enhance(1.2)

    return img


# ── Step 5: Normalize and prepare for model ───────────────────────────────────
def normalize(img: Image.Image) -> np.ndarray:
    img = img.resize((256, 256), Image.LANCZOS)
    arr = np.array(img, dtype=np.float32) / 255.0
    # ImageNet-style mean/std normalization (standard for CNN+ViT hybrid models)
...
def full_preprocess(image_bytes: bytes) -> tuple:
    ...
    Returns (model_input, metadata) or raises HTTPException.
    """
    # Decode image
    try:
